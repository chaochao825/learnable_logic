...
| bitplane_lut_sequence_causal_argmax | candidate | 512 protected context planes plus character-vote 4-input LUT planes with recent-history and same-class recurrent candidates | eight Boolean planes per character ID | block-wise hard-ST then direct argmax and hard freeze | bool LUT/wiring plus integer GroupSum | tiny_shakespeare_char 28.99% on validation (screen); test 28.50% | matched smoke improves validation hard accuracy by 3.17 pp and reduces unused gates by 0.53 pp but remains below the integer trigram reference; full width scaling is in progress |
...
| bitplane_lut_sequence_causal_argmax | shakespeare_causal_v32_d2 | 512 | 512 | 2592 | 332800 | 304128 | 4160 | 2 | 100 | same gate and payload budget as mixed; candidates encode recent causal positions and same-class vote recurrence |
