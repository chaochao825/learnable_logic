...
def parse_args():
    ...
    logic_only.add_argument("--head-type", choices=["linear", "group_sum", "group_sum_bias", "nonnegative_linear", "signed_sparse_linear", "signed_counter_linear", "scaled_signed_counter_linear"], default="linear", help="Classifier/readout head type")
    logic_only.add_argument("--signed-head-topk", type=int, default=32, help="Per-class top-k kept when signed sparse/counter heads are masked")
    logic_only.add_argument("--signed-head-use-topk-mask", action=argparse.BooleanOptionalAction, default=False, help="Apply signed sparse/counter top-k mask during forward")
...
def train():
    ...
    if getattr(args, "head_type", "linear") in {"signed_sparse_linear", "signed_counter_linear", "scaled_signed_counter_linear"}:
        apply_signed_sparse_head_mask(model)
        msg_sparse = f"Applied {args.head_type} top-k mask: topk={args.signed_head_topk}"
        print(msg_sparse)
        append_log(msg_sparse)
