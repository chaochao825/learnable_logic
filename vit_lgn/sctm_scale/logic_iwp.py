...
class LogicLayerIWP(BaseLogicLayerIWP):
    ...
    **与父类的区别**：
    ...
    def set_soft_eval(self, soft: bool):
        ...
        Note:
            - 在 `_forward_cuda_with_temperature()` 中，训练默认使用软评估；但如果
              `set_train_hard_st(True)`，训练前向会使用 hard truth-table bits with STE
            - 只有在推理时（`self.training=False`）且 `_soft_eval=False` 时，才会使用硬评估
        """
        self._soft_eval = bool(soft)

    def set_train_hard_st(self, enabled: bool):
        """Use hard truth-table bits in training forward with STE gradients."""
        self._train_hard_st = bool(enabled)
    ...
    def _forward_cuda_with_temperature(self, x: torch.Tensor) -> torch.Tensor:
        ...
        **计算流程**：
        ...
        probs = self.act_fn(logits)
        use_soft_weights = not (self.training and self._train_hard_st) and (
            self._soft_eval or self.training
        )
        if use_soft_weights:
            w = probs.to(x.dtype)
        else:
...
class LearnableConnLightLogicLayer(nn.Module):
    ...
    def _build_candidates(self, generator: torch.Generator | None) -> tuple[torch.Tensor, torch.Tensor]:
        counts = {
            "prev": int(round(self.candidate_k * 0.60)),
            "prev2": int(round(self.candidate_k * 0.25)),
            "older": int(round(self.candidate_k * 0.10)),
        }
        counts["input"] = self.candidate_k - sum(counts.values())
        sources = {
        ...
        offset = 0
        for key in ("prev", "prev2", "older", "input"):
            count = counts[key]
            if count <= 0:
                continue
    ...
    def _scores(self, logits: torch.Tensor, layers: torch.Tensor) -> torch.Tensor:
        penalty = (self.layer_id - layers).to(dtype=logits.dtype, device=logits.device)
    ...
    def _soft_port(
        ...
        values = self._gather(history, layers, nodes)
        weights = torch.softmax(self._scores(logits, layers) / self.tau_conn, dim=-1).to(values.dtype)
    ...
    def connection_metrics(self) -> dict[str, float]:
        with torch.no_grad():
            ...
            distances = (self.layer_id - selected_layers).float()
    ...
    def set_train_hard_st(self, enabled: bool) -> None:
        self.gate.set_train_hard_st(enabled)
...
class LogicFFN_IWP(nn.Module):
        ...
        if self.use_thermometer:
            ...
            for layer in self.layers:
                if hasattr(layer, "forward_from_history"):
                    h_binary = layer.forward_from_history(history)
                else:
            ...
            for layer in self.layers:
                if hasattr(layer, "forward_from_history"):
                    h = layer.forward_from_history(history)
                else:
                    h = layer(h)
                h = self.dropout(h)
                history.append(h)
    ...
    def connection_stats(self) -> dict[str, float]:
        learnable_layers = [layer for layer in self.layers if hasattr(layer, "connection_metrics")]
        ...
        for layer in learnable_layers:
            ...
            fanout_keys.append(l0_cpu * self.layers[0].in_dim + n0_cpu)
            fanout_keys.append(l1_cpu * self.layers[0].in_dim + n1_cpu)
            dead_sum += layer.gate_dead_ratio() * layer.out_dim
        ...
        if all_keys.numel() > 0:
            fanout = torch.bincount(all_keys)
            fanout = fanout[fanout > 0].float()
            stats["conn_fanout_max"] = float(fanout.max().item())
            stats["conn_fanout_p95"] = float(torch.quantile(fanout, 0.95).item())
