...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Small-budget ViT-LGN Goal 6+ hard-ST student experiment."
    )
    ...
    parser.add_argument("--head-type", choices=["linear", "group_sum", "group_sum_bias", "nonnegative_linear", "signed_sparse_linear", "signed_counter_linear", "scaled_signed_counter_linear"], default="linear")
    parser.add_argument("--signed-head-topk", type=int, default=32)
    parser.add_argument("--signed-head-use-topk-mask", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--post-mask-head-calibrate-iters", type=int, default=0)
    parser.add_argument("--post-mask-head-calibrate-lr", type=float, default=0.0)
    parser.add_argument(
        "--post-mask-head-calibrate-target",
...
def make_train_args(args: argparse.Namespace) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        signed_head_use_topk_mask=args.signed_head_use_topk_mask,
...
def set_logic_train_hard_st(model: nn.Module, enabled: bool) -> None:
    for module in model.modules():
        if hasattr(module, "set_train_hard_st"):
            module.set_train_hard_st(enabled)
...
def apply_signed_sparse_head_mask(model: nn.Module) -> None:
    head = getattr(model, "head", None)
    if hasattr(head, "apply_topk_mask"):
        head.apply_topk_mask()


def calibrate_masked_head(
    model: nn.Module,
    train_loader,
    transform,
    args,
    ...
    iters = int(getattr(args, "post_mask_head_calibrate_iters", 0))
    if iters <= 0:
        return 0.0, 0
    head = getattr(model, "head", None)
    if head is None or not hasattr(head, "effective_weight"):
        return 0.0, 0
    use_mask = getattr(head, "use_topk_mask_flag", torch.tensor(False))
    if not bool(use_mask.item()):
        return 0.0, 0
    ...
    counter_scale = getattr(head, "counter_scale", None)
    scaled_flag = getattr(head, "scaled_counter_flag", torch.tensor(False))
    if counter_scale is not None and bool(scaled_flag.item()):
        counter_scale.requires_grad_(True)
        params.append(counter_scale)
    if getattr(head, "bias", None) is not None:
        head.bias.requires_grad_(True)
        params.append(head.bias)
    if not params:
        return 0.0, 0

    lr = (
        float(args.post_mask_head_calibrate_lr)
        if args.post_mask_head_calibrate_lr > 0
        else float(args.student_lr)
    )
    optimizer = torch.optim.AdamW(params, lr=lr, weight_decay=0.0)
    iterator = iter(train_loader)
    start = time.time()
    set_model_temperature(model, args.temp_end)
    tau_conn, beta_skip, _freeze_conn = get_connection_schedule(
        train_args.num_iterations,
        max(train_args.num_iterations, 1),
        train_args,
    )
    set_connection_schedule(model, tau_conn, beta_skip, True)
    ...
    for _step in range(iters):
        ...
        prepare_forward(model, "hard_forward")
        ...
        if counter_scale is not None and bool(scaled_flag.item()):
            with torch.no_grad():
                counter_scale.clamp_(min=1e-6)

    for param in params:
        param.requires_grad_(False)
    return time.time() - start, sum(param.numel() for param in params)
...
def prepare_forward(model: nn.Module, mode: str) -> None:
    set_logic_train_hard_st(model, False)
...
def train_student(
    ...
    for step in range(args.student_iters):
        ...
        set_logic_train_hard_st(student, True)
    ...
    set_logic_train_hard_st(student, False)
...
def gate_stats(model: nn.Module) -> dict[str, float]:
    ...
    constant = 0
    fanout_max = 0
    layers = list(logic_layers(model))
    with torch.no_grad():
        for layer in layers:
...
def main() -> None:
    ...
    for method, model, mode, permanent in eval_specs:
        ...
        row = {
            ...
            "post_mask_head_calibrate_iters": args.post_mask_head_calibrate_iters,
            "post_mask_head_calibrate_lr": args.post_mask_head_calibrate_lr,
            "post_mask_head_calibrate_target": args.post_mask_head_calibrate_target,
