...
LOG_DIR="logs/20260628_cls_topk_token_mixer_b123"
mkdir -p "${LOG_DIR}"
RUN_STAMP="$(date '+%Y%m%d_%H%M%S')_pid$$"
LOG_FILE="${LOG_DIR}/run_${RUN_STAMP}.log"

{
    echo "cls top-k token mixer b1-b3 ablation start $(date -Is)"
    echo "python=${PY}"
    ...
    "${PY}" cls_topk_token_mixer_ablation.py \
        ...
        --block-settings "b1,b2,b3" \
        --topk-list 4,8,16 \
        --mixer-variants static_mean,static_weighted,dynamic_mean,dynamic_weighted \
        --out-dir runs/cls_topk_token_mixer_b123_topkmasked \
        --num-workers 2
    echo "cls top-k token mixer b1-b3 ablation done $(date -Is)"
} 2>&1 | tee "${LOG_FILE}"
