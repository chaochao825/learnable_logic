...
class LogicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ffn = LogicFFN_IWP(
            ...
            cross_block_source_count=cross_block_source_count,
            cross_block_candidate_frac=cross_block_candidate_frac,
        )
    ...
    def forward_with_logic_history(
        self,
        x: torch.Tensor,
        external_history: list[torch.Tensor],
    ) -> tuple[torch.Tensor, torch.Tensor]:
        x = x + self.drop_path(self.attn(self.norm1(x)))
        ffn_out, logic_state = self.ffn.forward_with_logic_history(self.norm2(x), external_history)
        x = x + self.drop_path(ffn_out)
        return x, logic_state
...
class LogicViTTiny(nn.Module):
    ...
    def __init__(
        ...
        learnable_conn_k: int = 64,
        learnable_conn_use_skip_bias: bool = True,
        cross_block_logic_history: bool = False,
        cross_block_candidate_frac: float = 0.0,
    ):
        """
        初始化逻辑版 ViT-Tiny 模型。
        ...
        """
        super().__init__()
        self.cross_block_logic_history = bool(cross_block_logic_history)
        ...
        for i in range(depth):
            ...
            if block_cls is LogicTransformerBlock:
                ...
                kwargs["cross_block_source_count"] = i if self.cross_block_logic_history else 0
                kwargs["cross_block_candidate_frac"] = cross_block_candidate_frac if self.cross_block_logic_history else 0.0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 编码器
        if self.cross_block_logic_history:
            logic_history: list[torch.Tensor] = []
            for blk in self.blocks:
                if hasattr(blk, "forward_with_logic_history"):
                    x, logic_state = blk.forward_with_logic_history(x, logic_history)
                    logic_history.append(logic_state)
                else:
                    x = blk(x)
        else:
            for blk in self.blocks:
...
def logic_vit_tiny(**kwargs) -> LogicViTTiny:
    ...
    model = LogicViTTiny(
        ...
        cross_block_logic_history=kwargs.get("cross_block_logic_history", False),
        cross_block_candidate_frac=kwargs.get("cross_block_candidate_frac", 0.0),
    )
    return model
