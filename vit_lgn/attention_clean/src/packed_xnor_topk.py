...
class PackedXnorTopK(nn.Module):
    ...
    def __init__(
        ...
        if topk_forward_mode not in ("topk", "random-k", "fixed-random-k"):
            raise ValueError("topk_forward_mode 只能是 'topk'、'random-k' 或 'fixed-random-k'")
    ...
    @staticmethod
    def _pad_last_dim(x: torch.Tensor, target_len: int, pad_value: int) -> torch.Tensor:
    ...
    def forward(
        ...
        row_count = q.shape[-2]
        limit = min(topk, row_count)
        topk_indices: torch.Tensor
        selector_mask = None

        if self.topk_forward_mode == "random-k":
            topk_indices = self._random_k_indices(xnor_popcount, limit)
        elif self.topk_forward_mode == "fixed-random-k":
            topk_indices = self._fixed_random_k_indices(xnor_popcount, limit)
        elif self.topk_impl == "torch-topk":
            topk_indices = self._torch_topk_indices(xnor_popcount, limit)
        else:
            base_indices = torch.arange(row_count, device=q.device, dtype=torch.int64)
            view_shape = [1] * xnor_popcount.ndim
            view_shape[-1] = row_count
            base_indices = base_indices.view(*view_shape).expand_as(xnor_popcount)

            _, topk_indices = self._winner_tree_topk(xnor_popcount, base_indices, limit)

        if return_selector_mask:
            selector_mask_hard = self._build_selector_mask(topk_indices, q.shape[-2])

            use_surrogate = self.training and (q.requires_grad or k.requires_grad) and (q.is_floating_point() or k.is_floating_point())
            if use_surrogate:
